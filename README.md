# Excel-Sales-Analytics
Here are all the reports that I created while learning Excel from CodeBasics.

# 📊 Business Analytics Reports (Excel Portfolio)

This repository contains a collection of business intelligence and financial analysis reports built in Microsoft Excel. The project demonstrates practical data analysis skills used in business analytics, finance reporting, and performance monitoring.
The reports focus on transforming raw business data into actionable insights using dashboards, pivot tables, financial models, and data visualization.

📁 Project Reports ***

# 📈 Financial Analysis & Profitability
These reports analyze profitability, margins, and financial performance across different time periods and markets.

## P & L by Fiscal Year - https://github.com/Sushant7274/Excel-Sales-Analytics/blob/main/P%26L%20Statement%20by%20Fiscal%20Year.pdf 
## P&L Statement by Months.pdf
## P&L Statement by Markets.pdf
## Gross Margin % by Quarters.pdf

# 📊 Key Insights Covered

1. Revenue vs cost breakdown
2. Profit trends over time
3. Market profitability comparison
4. Margin analysis

# 🌍 Market & Operational Performance
These reports track business performance across markets, divisions, and booking channels.

# All Properties Performance.pdf
# Division Level Report.pdf
# Market Performance vs Target Report.pdf
# Booking Platforms By week.pdf

# 📊 Key Insights Covered

1. Target vs actual performance
2. Weekly platform activity
3. Market growth comparison
4. Division-level business performance

# 🛍 Product & Customer Analytics
These reports analyze customer behavior, product performance, and regional trends.

## Customer Performance Report.pdf
## Net Products 2021.pdf
## Top 10 Products.pdf
## Top 5 Countries.pdf
## Top 5 and bottom 5.pdf

# 📊 Key Insights Covered

1. Top performing products
2. Underperforming products
3. Customer value analysis
4. Geographic sales performance

# 🧠 Strategy, Planning & Decision Support
These models help support business decision-making and project prioritization.

## Project Priority Matrix.pdf 
## Scenario Planning Final.pdf
## Loan_repayment.xlsx
## Expense Project.xlsx

# 📊 Key Insights Covered

1. Resource allocation decisions
2. Financial planning scenarios
3. Budget tracking
4. Project prioritization

# 🛠 Skills Demonstrated
This portfolio highlights the following data analytics and Excel skills:

1. Pivot Tables & Pivot Charts
2. Financial Modeling
3. KPI Reporting
4. Business Performance Analysis
5. Data Visualization
6. Scenario & What-If Analysis
7. Dashboard Creation
8. Data Cleaning & Structuring

# 📊 Tools Used

1. Microsoft Excel
2. Pivot Tables
3. Excel Charts & Visualization
4. Financial Modeling Techniques
5. Business KPI Frameworks.
